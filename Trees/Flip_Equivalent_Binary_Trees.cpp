...
bool flipEquiv(TreeNode* root1, TreeNode* root2) {
    ...
    if(root1->val != root2->val)
        return false;
    bool left = (flipEquiv(root1->left,root2->left) && flipEquiv(root1->right,root2->right));
    bool right = (flipEquiv(root1->left,root2->right) && flipEquiv(root1->right,root2->left));
    return left || right;
}
